...
public abstract class PortabilityTestBase : SystemTestBase
{
    ...
    private void RunSkippableTest(Action test)
    {
        ...
        if (_testRunConfiguration is { TargetFramework: TargetFramework.Net462 or TargetFramework.Net472 or TargetFramework.Net60 or TargetFramework.Net70, UnitTestProvider: UnitTestProvider.xUnit3 })
            Assert.Inconclusive("Disabled because xUnit3 is not supported on .NET 4.6.2, .NET 4.7.2, .NET 6.0 or .NET 7.0");
